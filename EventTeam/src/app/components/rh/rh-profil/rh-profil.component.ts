...
export class RhProfilComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.responsableRhService.getProfile().subscribe({
      next: (profile) => {
  ...
  onPhotoSelected(event: Event): void {
    ...
    if (file.size > RhProfilComponent.MAX_PHOTO_SIZE) {
    ...
    reader.onload = () => {
      ...
      this.responsableRhService.updatePhoto(base64).subscribe({
        next: (data) => {
  ...
  onDeletePhoto(): void {
    this.photoUploading = true;
    this.photoError = '';
    this.responsableRhService.deletePhoto().subscribe({
      next: () => {
        this.photo = null;
        this.photoUploading = false;
      },
